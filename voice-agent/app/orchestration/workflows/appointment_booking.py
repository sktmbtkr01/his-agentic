...
class AppointmentBookingWorkflow(BaseWorkflow):
    ...
    async def _book_appointment(
        ...
        # Fix: If patient_id is provided but is a custom ID (e.g. PAT000064), resolve it to ObjectId
        if patient_id and not len(str(patient_id)) == 24:
            try:
                logger.info("Resolving custom patient ID to ObjectId", custom_id=patient_id)
                patients = await self.his_client.search_patients(query=patient_id)
                if patients:
                    ...
                    if not resolved_patient:
                        resolved_patient = patients[0]
                        
                    patient_id = resolved_patient.get("_id")
                    patient = resolved_patient
                    logger.info("Resolved patient ID", original=patient_id, resolved=resolved_patient.get("_id"))
                else:
                    logger.warning("Could not resolve custom patient ID", custom_id=patient_id)
            except Exception as e:
                logger.error("Error resolving patient ID", error=str(e))
